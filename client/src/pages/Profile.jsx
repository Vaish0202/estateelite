...
export default function Profile() {
  ...
  const [showListingsError, setSetshowListingsError] = useState(false)
  ...
  const handleShowListing = async () => {
    try {
      setSetshowListingsError(false);
      const res = await fetch(`/api/user/listings/${currentUser._id}`);
      const data = await res.json();
      if(data.success === false){
        setSetshowListingsError(true);
        return;
      }
  ...
  const handleListingdelete =async (listingId) => {
    try {
      ...
      setGetUserListings((prev) => prev.filter((listing) => listing._id !== listingId));
    } catch (error) {
  ...
  return (
    ...
    {fileUploadError ? (
      ...
      Upload (Image must be less than 2MB)</span>
        ...
        <Link className="bg-green-700 text-white p-3 rounded-lg uppercase text-center hover: opacity-95" to={"/create-listing"}>Add Property</Link>
      ...
      <div className="flex flex-col gap-4">
        <h1 className="text-center text-2xl font-semibold">Your Properties</h1>
        {getUserListings.map((listing) => <div key={listing._id} 
       className="border rounded-lg p-3 flex justify-between items-center gap-4">
        <Link to={`/listing/${listing._id}`}>
          <img src={listing.imageUrls[0]} alt="lisiting cover" className="h-16 w-16 object-contain " />
          </Link>    
